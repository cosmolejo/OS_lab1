...
void report(list_t *list, FILE *output)
{
	node_t *tmp;
	tmp = list->head;
	fprintf(output, "-- Información recolectada!!!\n");
	while (tmp != NULL)
	{
		fprintf(output, "PID: %s\n", tmp->datos.pid);
		fprintf(output, "Nombre del proceso: %s\n", tmp->datos.name); // Finalmente se imprime la información extraída del archivo
		fprintf(output, "Estado: %s\n", tmp->datos.state);
		fprintf(output, "Tamaño total de la imagen de memoria: %s\n", tmp->datos.vmSize);
		fprintf(output, "\tTamaño de la memoria de la región TEXT:  %s\n", tmp->datos.vmData);
		fprintf(output, "\tTamaño de la memoria de la región DATA: %s\n", tmp->datos.vmStk);
		fprintf(output, "\tTamaño de la memoria de la región STACK: %s\n", tmp->datos.vmExe);
		fprintf(output, "Número de cambios de contexto realizados (voluntarios - no voluntarios): %s - %s\n",
				tmp->datos.v_ctx_swt, tmp->datos.nv_ctx_swt);
		tmp = tmp->next;
	}
}
